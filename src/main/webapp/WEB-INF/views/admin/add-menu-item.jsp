...
<!DOCTYPE html>
<html>
<head><title>Add Menu Item | Pokhara Bites</title><link rel="stylesheet" href="${pageContext.request.contextPath}/css/style.css"></head>
<body><jsp:include page="/components/header.jsp"/>
<main class="page narrow">
    <p class="eyebrow">Admin</p><h1>Add Menu Item</h1>
    <c:if test="${not empty error}"><div class="alert error"><c:out value="${error}"/></div></c:if>
    <form method="post" action="${pageContext.request.contextPath}/admin/menu-items/add" class="form-card">
        <label>Restaurant
        ...
        <label>Availability
            <select name="availability">
                <option value="true" ${empty item || item.available ? 'selected' : ''}>Yes</option>
                <option value="false" ${not empty item && not item.available ? 'selected' : ''}>No</option>
            </select>
        </label>
        <button class="primary" type="submit">Save Menu Item</button>
    </form>
